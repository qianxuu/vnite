...
async function upscaleBackgroundImage(
  imageSource: Buffer | string,
  upscaleScale?: number
): Promise<Buffer | string> {
  if (!upscaleScale || upscaleScale <= 0) {
    return imageSource
  }
  ...
  try {
    ...
    if (upscalerPath) {
      const imageString = typeof imageSource === 'string' ? imageSource : imageSource.toString()
      return await upscaleImage(imageString, upscalerPath, { scale: upscaleScale })
    }
  } catch (err) {
